...
tvc_view3d.py
=============
Animated 3D view of a simulation run produced by tvc_control.physics.simulate().
...
Opened from the "3D View" button in tvc_gui.py, but also runnable standalone:

    python3 tvc_view3d.py          # runs the default case and animates it
...
CACHE_DIR = os.path.join(HERE, "sim", "models", "tvc_vehicle", "meshes")
...
def _vehicle_cg():
    ...
    Falls back to (0, 0, L) when the sim/ tree isn't reachable."""
...
def load_vehicle_mesh(params: VehicleParams, detail=DEFAULT_DETAIL):
    ...
    try:
        ...
        print(f"[tvc_view3d] could not load {STL_PATH}: {e}; using placeholder body")
        return None
