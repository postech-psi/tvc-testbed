...
// 링키지 비율 반영
// hinge 10deg = roll servo 24deg, yaw servo 15deg
...
void setup() {
  ...
  // 천천히 3회 반복
  for (int i = 0; i < 1; i++) {
    sweepAxis(yaw, 90, PITCH_SERVO_10DEG, 30);
    delay(100);
  }
  hold(yaw, 90, 1000);

  // --- 2단계: roll 단독 (hinge ±10°) ---
  for (int i = 0; i < 1; i++) {
    sweepAxis(roll, 90, YAW_SERVO_10DEG, 30);
    delay(100);
  }
  hold(roll, 90, 1000);
  ...
  for (int lap = 0; lap < 15; lap++) {
    for (int deg = 0; deg <= 360; deg += 2) {
      ...
      yaw.write(90 + (int)(PITCH_SERVO_10DEG * sin(rad)));
      roll.write(90   + (int)(YAW_SERVO_10DEG   * cos(rad)));
      delay(4);
    }
  }
