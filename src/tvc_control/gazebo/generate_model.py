"""Build Gazebo XML from manually entered vehicle settings. Never reads CAD geometry."""
import xml.etree.ElementTree as ET

import numpy as np

from tvc_control import config as vp
...
# Gimbal plugin gains: d*dt/I = 2*zeta*wn*0.001 = 0.64.
# These differ from the analytic servo delay/slew model; see docs/GUIDE.md.
WORLD_STEP_S = 0.001
...
def render(v, base_mass, base_pos, I_base_own):
    ...
    # The joint velocity limit is above measured slew. Gazebo currently applies
    # only this plugin model, so the analytic per-ring slew is not reproduced.
    ...
    def motor_plugin(joint, link, turn, num):
      ...
      <!-- This plugin filters rotor VELOCITY. Neither Gazebo entry path
           applies the analytic MotorLag, so its 100 ms thrust/torque response
           is absent here. Since T ~ omega^2, this speed filter is not a
           first-order lag in thrust. Dynamic plant parity remains open. -->
    ...
    def servo_plugin(joint, topic, ring):
      ...
      <!-- DERIVED, not chosen. The {ring} ring swings {inertia:.3e} kg.m^2 about its
           ...
           gimbal dynamics live in physics/actuators.py, not here -- this servo
           only has to be quick enough to disappear next to them.
           See generate_model.py's SERVO_* block. -->
      <p_gain>{kp:.4f}</p_gain>
      <i_gain>0.0</i_gain>
      <d_gain>{kd:.5f}</d_gain>
...
<!--
  ...
  Regenerate with:  python gazebo/generate_model.py
  Source of truth:  tvc_control/settings/vehicle.yaml
  ...
  gazebo/generate_model.py TOKEN_LINKS) subtracted from base_link, so this model no
...
{inertial(base_mass, bz, Ib)}
    ...
    <!-- ============ plugins (constants from settings/vehicle.yaml) ========= -->
...
def model_xml(mesh_uri):
    """Create the model for a launch, using the configured mass and inertia."""
    v = vp.load()
    base_mass, base_pos, inertia = solve_base_link(v)
    model = _xml_safe(render(v, base_mass, base_pos, inertia))
    return model.replace("model://tvc_vehicle/meshes/tvc_vehicle.stl", mesh_uri)
