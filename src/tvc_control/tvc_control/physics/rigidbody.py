"""Six-degree-of-freedom rigid-body equations with a full inertia tensor."""

import numpy as np

from ..control.params import VehicleParams
from ..control.mathx import quat_normalize, quat_to_rotmat, thrust_axis
...
def quat_kinematics(q, omega):
    ...
    needs it, and keeping it here is what lets control/mathx.py stay free of numpy.
    """
    wx, wy, wz = omega
    Omega = np.array([
        [0, -wx, -wy, -wz],
...
def dynamics(t, x, T, delta, params: VehicleParams, tau_p=0.0):
    ...
    # AERODYNAMICS (opt-in, ESTIMATED -- see physics/aero.py). Off by default, in
    ...
    # [ixy, iyy, iyz], [ixz, iyz, izz]], which is what generate_model.py emits
