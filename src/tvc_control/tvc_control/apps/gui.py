"""
The interactive front end to the analytic harness.
================================================================================
    python tvc.py gui

Edit vehicle parameters, control gains, and the initial/target attitude in the
form on the left; click Run Simulation; watch attitude, body rates and gimbal
command update in the plots on the right, with a metrics summary underneath.

It drives harness/mil.py -- the same analytic plant, flight code and gains as
`tvc.py validate`. Nothing here computes physics; it is a form and four axes.

Needs a display. numpy, scipy, matplotlib and tkinter only.

The fields are declared as (label, attribute) pairs below, so the form is a
projection of the dataclasses rather than a hand-written duplicate of them; a
renamed field is one edit here rather than a silent AttributeError at startup.
Only a subset is exposed -- the roll-channel gains, the altitude cascade and the
motor-lag switch are set in control_gains.yaml and vehicle_params.yaml, where
