"""Python simulation loop: estimate state, calculate commands, apply actuator dynamics, integrate motion."""

import numpy as np
from scipy.integrate import solve_ivp
from dataclasses import dataclass

from .control.params import VehicleParams, ControlGains
from .control.mathx import quat_normalize, quat_to_euler, euler_to_quat
from .control.types import ControlMode, Setpoint
from .control.controller import TvcController
from .physics.rigidbody import dynamics
from .physics.actuators import ActuatorChain
from .physics.sensors import PerfectEstimator


@dataclass
class SimConfig:
    """Run settings shared by the command line and GUI; SI units except angles."""

    t_final: float = 5.0
    dt_ctrl: float = 0.01
    att_pitch_des_deg: float = 0.0
    att_yaw_des_deg: float = 0.0
    att_roll_des_deg: float = 0.0
    ...
    motor_tau_s: float = None
    motor_deadtime_s: float = None
    battery_sag: bool = False

    def __post_init__(self):
        from .config import load
        settings = load().motor_dynamics
        if self.motor_model is None:
            self.motor_model = settings["model"]
        if self.motor_tau_s is None:
            self.motor_tau_s = settings["tau_s"]
        if self.motor_deadtime_s is None:
            self.motor_deadtime_s = settings["deadtime_s"]
...
def simulate(vparams: VehicleParams, gains: ControlGains, cfg: SimConfig):
    ...
    """
    if not (np.isfinite(cfg.t_final) and np.isfinite(cfg.dt_ctrl)
            and cfg.t_final > 0 and cfg.dt_ctrl > 0):
        raise ValueError("Duration and control period must be finite and positive.")
    battery, current_per_n = None, 0.0
    if cfg.battery_sag:
        # Reading the YAML here (not in control/) is fine: this is the harness, and
    ...
    for k in range(n_steps):
        ...
        # even though today's estimator is the identity. See physics/sensors.py.
