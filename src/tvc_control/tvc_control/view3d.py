"""Animate simulated motion using the final CAD mesh. The mesh is visual only."""
...
from .config import gazebo_directory
from .control.mathx import quat_to_rotmat
from .control.params import VehicleParams

STL_PATH = str(gazebo_directory() / "models/tvc_vehicle/meshes/tvc_vehicle.stl")
CACHE_DIR = os.path.join(tempfile.gettempdir(), "tvc-view3d")

# The SDF declares <scale>0.001 0.001 0.001</scale> -- the STL is authored in mm.
MESH_SCALE = 0.001

# Vertex-clustering cell size [m] per detail level. The final CAD mesh is 141k
...
def load_vehicle_mesh(params: VehicleParams, detail=DEFAULT_DETAIL):
    ...
    Each detail level's result is cached in the system temporary directory, so only the first open
    ...
    identity = (STL_PATH, os.stat(STL_PATH).st_mtime_ns, _vehicle_cg().tolist(), cell)
...
class View3DWindow(tk.Toplevel):
    ...
    def show_frame(self, k):
        ...
        # returns a tuple of rows, because control/ may not import numpy. This
        ...
        # frame -- they have not opened since the math moved into control/.
